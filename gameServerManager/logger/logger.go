...
func LoadLogger() error {
	//The 'Log' folder will always be in the root directory of Narwhal
	err := checkDirectory()
	if err != nil {
		return err
	}
	...
	logFile, err := os.OpenFile(fmt.Sprintf("Log/%s-narwhal.log", fileName), os.O_WRONLY|os.O_CREATE|os.O_APPEND, 0666)
